...
export default {
  ...
  setup () {
    ...
    const getDetail =  async () => {
      const { data } = await getProductDetail({ code: route.params.code })
      detail.value = data
      showPage.value = true
      propKeys.value = Object.keys(detail.value.properties)
    }

    const onChange = (index) => this.current = index
    ...
    return { current, propKeys, propKey, showPage, detail, getDetail, onChange, propName }
  }
}
</script>
